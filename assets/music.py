...
async def create_embed_play(data,queue=False):
    ...
    embed = discord.Embed(title='Song found!', description=f'```css\n{data["uploader"].replace(" - Topic","")} - {data["title"]}\n```',color=config.embed_color['play'],timestamp=datetime.datetime.now())
    if queue:
        embed = discord.Embed(title='Song added to the queue!', description=f'```css\n{data["uploader"].replace(" - Topic","")} - {data["title"]}\n```',color=config.embed_color['queue'],timestamp=datetime.datetime.now())
